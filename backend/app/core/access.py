"""
Tenant and role scoping for signal files and reports.

Single source of truth for "which rows may this user see". Every endpoint that
reaches a SignalFile or an EEGReport by id goes through the dependencies here, so
the rule is written once. Four hand-copied versions of the file check had drifted

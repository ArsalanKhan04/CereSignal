...
def _ensure_logs_dir(path: Path) -> Path:
    try:
        path.mkdir(parents=True, exist_ok=True)
        return path
    except OSError:
        fallback = Path.home() / ".ceresignal" / "logs"
        fallback.mkdir(parents=True, exist_ok=True)
        return fallback


# Create logs directory
LOGS_DIR = _ensure_logs_dir(_resolve_logs_dir())
...
def setup_logging(app_name: str = "ceresignal") -> logging.Logger:
    ...
    keep_forever = os.getenv("CERE_LOG_KEEP_FOREVER") == "1"
    backup_count = 0 if keep_forever else 30

...
async def create_report(
    ...
    signal_file = db.query(SignalFile).join(User).filter(
    ...
    existing_report = db.query(EEGReport).filter(
        EEGReport.file_id == report_data.file_id
    ).first()
...
async def get_report_by_file(
    ...
    report = db.query(EEGReport).filter(
        EEGReport.file_id == file_id
    ).first()
